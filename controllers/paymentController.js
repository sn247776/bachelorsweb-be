...
export const buySubscription = catchAsyncError(async (req, res, next) => {
  ...
  const plan_id = process.env.PLAN_ID || "plan_LoiXrqAoobrO9F";
...
export const paymentVerification = catchAsyncError(async (req, res, next) => {
  ...
  if (!isAuthentic) return res.redirect(`${process.env.FE_URL}/paymentfail`);
  ...
  res.redirect(
    `${process.env.FE_URL}/paymentsuccess?reference=${razorpay_payment_id}`
  );
});
